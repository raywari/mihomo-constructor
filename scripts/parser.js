(function (global) {
  ...
  function parseVmess(u) {
    ...
    if (!rest.includes("@") && !u.search) {
      ...
      const obj = {
        name: sanitizeString(rawName, cfg.add || "vmess"),
        type: "vmess",
    ...
    const q = parsedQs(u);
    if (!password && q.encryption && u.username) {
      password = decodeURIComponent(u.username);
      method = stripControlChars(q.encryption);
    }
  ...
  function parseMany(raw, { collectErrors = false } = {}) {
